...
/*
 * Generated by python3's hashlib
 */
static const char *hash_48_0 =
	"e26a399ec356abf7b272274d3732ecdbb9e092db2549d43a3dfa01a68a8a990a"
	"5e7ceb861379c1e081af094f89f1df86";
...
START_TEST(test_blake2b)
{
	...
	blake2b_init(&state, key, sizeof(key), 64);
	blake2b_update(&state, buf, 0);
	blake2b_final(&state, buf);
	dump_hex(hex, buf, 64);
	ck_assert_str_eq(hash_64_0, hex);
	...
	blake2b_final(&state, buf);
	dump_hex(hex, buf, 64);
	ck_assert_str_eq(hash_64_1, hex);

	memset(buf, '\0', 70);
	blake2b_init(&state, NULL, 0, 48);
	blake2b_update(&state, "Lorem ipsum", 11);
	blake2b_final(&state, buf);
	dump_hex(hex, buf, 48);
	ck_assert_str_eq(hash_48_0, hex);
